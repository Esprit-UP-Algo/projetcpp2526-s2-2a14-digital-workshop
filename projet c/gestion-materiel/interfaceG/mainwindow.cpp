...
{
    // Ajouter des exemples de matériel
    Material mat1 = {1, "Poutre en chêne", "Bois", 50, 10, QDate::currentDate(), "BoisCorp", "Disponible"};
    Material mat2 = {2, "Marteau perforateur", "Outil", 15, 5, QDate(2024, 1, 15), "TechTools", "Disponible"};
    Material mat3 = {3, "Vis à bois 5x60", "Consommable", 500, 100, QDate(2024, 2, 20), "FixPro", "Disponible"};
    materialsList << mat1 << mat2 << mat3;
...
void MainWindow::setupUsersPage()
{
    ...
    userSearchEdit->setPlaceholderText("🔍 Rechercher...");
    ...
    headerLayout->addWidget(userSearchEdit);
    headerLayout->addWidget(searchBtn);
    ...
    userNameEdit->setPlaceholderText("Nom complet");
    userNameEdit->setStyleSheet(lineEditStyle);

    userEmailEdit = new QLineEdit;
    userEmailEdit->setPlaceholderText("email@entreprise.com");
    userEmailEdit->setStyleSheet(lineEditStyle);

    userPasswordEdit = new QLineEdit;
    userPasswordEdit->setPlaceholderText("Mot de passe");
    userPasswordEdit->setEchoMode(QLineEdit::Password);
    userPasswordEdit->setStyleSheet(lineEditStyle);

    userRoleBox = new QComboBox;
    userRoleBox->addItems({"Admin", "Responsable", "Ouvrier", "Technicien"});
    userRoleBox->setStyleSheet(comboBoxStyle);

    userStatusBox = new QComboBox;
    userStatusBox->addItems({"Actif", "Inactif"});
    userStatusBox->setStyleSheet(comboBoxStyle);

    // Champ pour la date de création (caché mais utilisé pour le tri)
    userCreationDateEdit = new QDateEdit;
    userCreationDateEdit->setDate(QDate::currentDate());
    userCreationDateEdit->setVisible(false);
    ...
    inputForm->addRow(idLabel, userIdEdit);
    inputForm->addRow(nameLabel, userNameEdit);
    inputForm->addRow(emailLabel, userEmailEdit);
    inputForm->addRow(passwordLabel, userPasswordEdit);
    inputForm->addRow(roleLabel, userRoleBox);
    inputForm->addRow(statusLabel, userStatusBox);

    // Configurer la taille des champs
    int fieldHeight = 38;
    userIdEdit->setFixedHeight(fieldHeight);
    userNameEdit->setFixedHeight(fieldHeight);
    userEmailEdit->setFixedHeight(fieldHeight);
    userPasswordEdit->setFixedHeight(fieldHeight);
    userRoleBox->setFixedHeight(fieldHeight);
    userStatusBox->setFixedHeight(fieldHeight);
    ...
    QPushButton *addBtn = createStyledButton("➕ Ajouter", "#2ecc71");
    userModifyBtn = createStyledButton("✏️ Modifier", "#3498db");
    QPushButton *clearBtn = createStyledButton("🔄 Effacer", "#95a5a6");

    // Taille fixe pour les boutons
    int buttonHeight = 38;
    addBtn->setFixedHeight(buttonHeight);
    userModifyBtn->setFixedHeight(buttonHeight);
    clearBtn->setFixedHeight(buttonHeight);

    // Largeur minimale pour les boutons
    addBtn->setMinimumWidth(120);
    userModifyBtn->setMinimumWidth(120);
    clearBtn->setMinimumWidth(120);

    formButtons->addWidget(addBtn);
    formButtons->addWidget(userModifyBtn);
    formButtons->addWidget(clearBtn);
    ...
    connect(userModifyBtn, &QPushButton::clicked, this, &MainWindow::modifyUser);
    // Reuse clearFields, but make sure it handles user fields too or create separate
    // For now create separate lambda or reuse if generic enough? Best to create specific if inputs differ.
    // Actually the existing clearFields is specific to Material. We need a clearUserFields.
    // For simplicity, let's just make a lambda or reusing a new slot?
    // Let's implement a clearUserFields inside the connection for now or rename.
    // Wait, the header has clearFields(). I should probably overload or make a new one.
    // No, better to keep them separate. I'll add clearUserFields as a lambda or new method if allowed by header.
    // Header doesn't have it. I'll use a lambda for now.
    connect(clearBtn, &QPushButton::clicked, [this](){
        userNameEdit->clear();
        userEmailEdit->clear();
        userPasswordEdit->clear();
        userRoleBox->setCurrentIndex(0);
        userStatusBox->setCurrentIndex(0);
        usersTable->clearSelection();
        userIdEdit->setText(QString::number(nextUserId));
    });

    connect(searchBtn, &QPushButton::clicked, this, &MainWindow::searchUser);
    connect(sortByDateBtn, &QPushButton::clicked, this, &MainWindow::sortUsersByDate);
    connect(sortByNameBtn, &QPushButton::clicked, this, &MainWindow::sortUsersByName);

    connect(usersTable, &QTableWidget::itemSelectionChanged, [this]() {
        int row = usersTable->currentRow();
        if (row >= 0) {
            userIdEdit->setText(usersTable->item(row, 0)->text());
            userNameEdit->setText(usersTable->item(row, 1)->text());
            userEmailEdit->setText(usersTable->item(row, 2)->text());
            userPasswordEdit->clear();

            // Mettre à jour les comboboxes
            QString role = usersTable->item(row, 3)->text();
            int roleIndex = userRoleBox->findText(role);
            if (roleIndex >= 0) userRoleBox->setCurrentIndex(roleIndex);

            QString status = usersTable->item(row, 4)->text();
            int statusIndex = userStatusBox->findText(status);
            if (statusIndex >= 0) userStatusBox->setCurrentIndex(statusIndex);
        }
    });
...
void MainWindow::addUser()
{
    ...
    newUser.email = userEmailEdit->text();
    newUser.role = userRoleBox->currentText();
    newUser.status = userStatusBox->currentText();
    newUser.creationDate = QDate::currentDate();
...
{
    int currentId = userIdEdit->text().toInt();
    bool found = false;

    for (User &user : usersList) {
        if (user.id == currentId) {
            user.name = userNameEdit->text();
            user.email = userEmailEdit->text();
            user.role = userRoleBox->currentText();
            user.status = userStatusBox->currentText();
            found = true;
            break;
        }
    }
    ...
    if (found) {
        ...
        QMessageBox::warning(this, "Erreur", "Utilisateur introuvable.");
    }
}
...
void MainWindow::deleteUser()
{
    int row = usersTable->currentRow();
    if (row < 0) {
        QMessageBox::warning(this, "Attention", "Veuillez sélectionner un utilisateur à supprimer.");
        return;
    }
    ...
    updateUsersTable();
    QMessageBox::information(this, "Succès", "Utilisateur supprimé.");
}

void MainWindow::searchUser()
{
    QString query = userSearchEdit->text().trimmed(); // Utiliser userSearchEdit, pas searchEdit (conflit potentiel)
    // Ah, wait, searchEdit is for Materials. userSearchEdit is new.
    ...
    for (const User &user : usersList) {
         if (user.name.contains(query, Qt::CaseInsensitive) || 
             user.email.contains(query, Qt::CaseInsensitive)) {
...
void MainWindow::sortUsersByDate()
{
    std::sort(usersList.begin(), usersList.end(), [](const User &a, const User &b) {
        return a.creationDate > b.creationDate; // Plus récent d'abord
    });
    updateUsersTable();
}

void MainWindow::sortUsersByName()
{
    std::sort(usersList.begin(), usersList.end(), [](const User &a, const User &b) {
        return a.name < b.name;
    });
