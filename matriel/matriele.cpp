    ...
    blinkState(false), nextId(1), dataModified(false)
{
    ...
    connect(serialTimer, &QTimer::timeout, this, &Matriele::readFromArduino);
...
void Matriele::setupMaterialPage() {
    ...
    thresholdEdit->setStyleSheet(thresholdEdit->styleSheet() + " background-color: #e9ecef; color: #8e44ad; font-weight: bold;");
    ...
    connect(materialTable, &QTableWidget::itemSelectionChanged, [this, deleteBtn]() {
        ...
        if (validSelection) {
            ...
            // Set and send the selected threshold back to Arduino
            int threshold = materialTable->item(row, 4)->text().toInt();
            thresholdEdit->setText(QString::number(threshold));
            sendToArduino(threshold);
...
void Matriele::fillThresholdFromArduino() {
#ifdef Q_OS_WIN
    ...
    bool accessDenied = false; // Add this to track if port exists but is blocked
    QString portName;
    
    // Check up to COM256 (Windows limit)
    for (int i = 1; i <= 256; ++i) {
        ...
        if (h != INVALID_HANDLE_VALUE) {
            ...
            arduinoFound = true;
            break; // Stop at first available COM port
        } else {
    ...
    if (arduinoFound) {
        ...
        showNotification("🔗 Connexion à l'Arduino (" + portName + ") réussie.", false);
    } else if (accessDenied) {
        showNotification("❌ Port " + portName + " détecté mais accès refusé.\nFermez le Moniteur Série de l'IDE Arduino !", true);
    } else {
        showNotification("❌ Aucun port COM détecté pour l'Arduino.", true);
    }
#else
    showNotification("❌ Fonctionnalité disponible uniquement sur Windows.", true);
#endif
}

void Matriele::readFromArduino() {
#ifdef Q_OS_WIN
    if (hSerial != INVALID_HANDLE_VALUE) {
        char buffer[256];
        DWORD bytesRead = 0;
        if (ReadFile(hSerial, buffer, sizeof(buffer) - 1, &bytesRead, NULL)) {
            if (bytesRead > 0) {
                buffer[bytesRead] = '\0';
                QString data = QString::fromStdString(buffer).trimmed();
                QStringList lines = data.split('\n', Qt::SkipEmptyParts);
                if (!lines.isEmpty()) {
                    bool ok;
                    int value = lines.last().trimmed().toInt(&ok);
                    if (ok) {
                        if (value > 9) value = 9;
                        thresholdEdit->setText(QString::number(value));
                        
                        QString originalStyle = thresholdEdit->styleSheet();
                        thresholdEdit->setStyleSheet(originalStyle + "border: 2px solid #2ecc71; background-color: #e8f0fe;");
                        QTimer::singleShot(500, [this, originalStyle]() {
                            thresholdEdit->setStyleSheet(originalStyle);
                        });
                    }
                }
            }
...
void Matriele::sendToArduino(int value) {
#ifdef Q_OS_WIN
    if (hSerial != INVALID_HANDLE_VALUE) {
        QString data = QString::number(value) + "\n";
        DWORD bytesWritten;
        WriteFile(hSerial, data.toStdString().c_str(), data.length(), &bytesWritten, NULL);
    }
#endif
}
