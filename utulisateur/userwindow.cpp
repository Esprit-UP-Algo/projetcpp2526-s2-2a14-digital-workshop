...

{
    ...
    navList->addItem("📈 Stats Clients");
    clientWin = new MainWindow(this);
   // clientWin->setSidebarVisible(false);
    pagesWidget->addWidget(clientWin);
    ...
    // Integration Commandes
    navList->addItem("🛒 Commandes");
    commandeWin = new CommandeWindow(this);
    pagesWidget->addWidget(commandeWin);

    // Gestion de la navigation personnalisée
    connect(navList, &QListWidget::currentRowChanged, [this](int row) {
        if (row == 0) { // Utilisateurs
            pagesWidget->setCurrentIndex(0);
        } else if (row == 1) { // Clients (Liste)
            pagesWidget->setCurrentIndex(1);
            clientWin->showListeClients();
        } else if (row == 2) { // Stats Clients
            pagesWidget->setCurrentIndex(1);
            clientWin->showStatistiques();
        } else if (row == 3) { // Matériel
            pagesWidget->setCurrentIndex(2);
        } else if (row == 4) { // Commandes
...
{
    // Mémoriser l'ID sélectionné
    int selectedId = -1;
    if (usersTable->currentRow() >= 0) {
        selectedId = usersTable->item(usersTable->currentRow(), 0)->text().toInt();
    }

    usersTable->setRowCount(0);
    int nextRowToSelect = -1;
    ...
    if (nextRowToSelect >= 0) {
        usersTable->selectRow(nextRowToSelect);
    } else {
        idEdit->setText(QString::number(nextId));
    }
}
...
void UserWindow::sortUsersByDate()
{
    ...
    if (databaseEnabled) {
        ...
        if (db.isOpen()) {
            ...
            QString sql = "SELECT ID_UTILISATEUR, NOM, EMAIL, TELEPHONE, MOT_DE_PASSE, ROLE, STATUT, "
                           "DATE_CREATION, COULEUR_FAVORITE, NOM_ANIMAL, MATRICULE_VOITURE, "
                           "PHOTO_PROFIL, A_VISAGE FROM UTILISATEUR ORDER BY DATE_CREATION ";
            sql += asc ? "ASC" : "DESC";
        ...
        std::sort(usersList.begin(), usersList.end(), [this](const User &a, const User &b) {
            return asc ? (a.creationDate < b.creationDate) : (a.creationDate > b.creationDate);
        });
        updateUsersTable();
    }
    asc = !asc;
...
void UserWindow::sortUsersByName()
{
    ...
    if (databaseEnabled) {
        ...
        if (db.isOpen()) {
            ...
            QString sql = "SELECT ID_UTILISATEUR, NOM, EMAIL, TELEPHONE, MOT_DE_PASSE, ROLE, STATUT, "
                           "DATE_CREATION, COULEUR_FAVORITE, NOM_ANIMAL, MATRICULE_VOITURE, "
                           "PHOTO_PROFIL, A_VISAGE FROM UTILISATEUR ORDER BY NOM ";
            sql += asc ? "ASC" : "DESC";
        ...
        std::sort(usersList.begin(), usersList.end(), [this](const User &a, const User &b) {
            if (a.name == b.name) return asc ? (a.id < b.id) : (a.id > b.id);
