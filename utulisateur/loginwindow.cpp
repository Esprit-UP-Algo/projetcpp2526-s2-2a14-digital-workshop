...
void LoginWindow::handleLogin()
{
    ...
    QSqlDatabase db = Connection::getInstance()->getDatabase();
    bool loginSuccessStatus = false;
    int userId = -1;

    // 1. Essayer la connexion Base de Données si elle est ouverte
    if (db.isOpen()) {
        QSqlQuery query(db);
        query.prepare(
            "SELECT ID_UTILISATEUR "
    ...
    // 2. Si échec DB ou DB fermée, vérifier les comptes de secours (Offline/Habituel)
    if (!loginSuccessStatus) {
        if ((email == "client@entreprise.com" && password == "tayssir1") ||
            (email == "admin@entreprise.com" && password == "admin123")) {
            userId = (email == "admin@entreprise.com") ? 1 : 2;
            loginSuccessStatus = true;
        } else if (email == "clients@entreprise.com" && password == "clients123") {
            userId = 3;
            loginSuccessStatus = true;
        }
    }
