...
void MainWindow::createListeClientsPage()
{
    ...
    radioEnCours->setStyleSheet(rb +
                                "QRadioButton::indicator:checked{background-color:#f59e0b;"
                                "border:2px solid #f59e0b;border-radius:8px;}");
    radioTermine->setStyleSheet(rb +
                                "QRadioButton::indicator:checked{background-color:#10b981;"
                                "border:2px solid #10b981;border-radius:8px;}");
    radioEnAttente->setStyleSheet(rb +
                                  "QRadioButton::indicator:checked{background-color:#3b82f6;"
    ...
    tableClients->setHorizontalHeaderLabels({
                                             "ID","Nom","Prenom","Email",
                                             "Telephone","Adresse","Date","Statut","Actions"});
...
void MainWindow::onExportPDF()
{
    ...
    QString html = "<h1 style='color:#1e40af'>"
                   ...
                   "<th>Tel</th><th>Adresse</th><th>Date</th><th>Statut</th></tr>";
    for (int r = 0; r < tableClients->rowCount(); ++r) {
...
void MainWindow::createStatistiquesPage()
{
    ...
    cl->addWidget(makeCard("En Cours",      statsEnCours,
                           "stop:0 #f093fb,stop:1 #f5576c"));
    cl->addWidget(makeCard("Terminees",     statsTerminees,
    ...
    gt->setStyleSheet("font-size:24px;font-weight:bold;"
                      "color:#1a202c;background:transparent;");
    gl->addWidget(gt);
...
{
    if (!statsClientTotal || !statsEnCours ||
        !statsTerminees   || !statsTable) return;
