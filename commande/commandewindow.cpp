    ...
    , ui(new Ui::CommandeWindow)
{
    ...
    if (ar.connect_arduino() == 0) {
        ...
        QMessageBox::warning(this, "Port", ar.getarduino_port_name());
...
void CommandeWindow::sendCommandeToLCD(int id_commande, QString status)
{            

    QString message = QString("%1;%3/")
                          .arg(id_commande)
                          .arg(status);  //5;En_attente/
       ...
       for (char c : data)
    {
        QByteArray byte;
        byte.append(c);              
        ar.write_to_arduino(byte);   
    }

    qDebug() << "LCD Arduino:" << message;
}
...
// appel du fonction du ajouter d'apres le classe
    ...
    commande c(idClient,date_commande, status,
              mode_paiement,
               prix);
...
//affichage du resultat ,
        ...
        if(test)
    {
        ...
        QFile file("C:/Users/amins/Desktop/QT_COMMANE/historique.txt");
...
void CommandeWindow::on_tableView_commande_clicked(const QModelIndex &index)
{
        ...
        QString  mode_paiement= model->data(model->index(row, 4)).toString();
        QString montant_totale = model->data(model->index(row, 5)).toString();
...
void CommandeWindow::on_bt_modifier_commande_clicked()
{
        ...
        commande c(id_commande,idClient,date_commande, status,
                   mode_paiement,
                   prix);
        ...
        QSqlQuery query_arduino;
        query_arduino.prepare("SELECT STATUT FROM COMMANDES WHERE ID_COMMANDE=:id");
        query_arduino.bindValue(":id",id_commande);
        if(query_arduino.exec() &&  query_arduino.next())
        {
          QString status_arduino= query_arduino.value(0).toString();
          sendCommandeToLCD(id_commande, status_arduino);
...
void CommandeWindow::on_Historique_clicked()
{
        ...
        QString filePath;
        QString modelHeader;
            filePath = "C:/Users/amins/Desktop/QT_COMMANE/historique.txt";
